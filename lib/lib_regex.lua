...
Lib.regex.key.begin = "^%s*begin%s*$" ---@type string Palavra-chave "begin"
Lib.regex.key.end_main = "^%s*begin%s*$" ---@type string Palavra-chave "end"
Lib.regex.key.end_method = "^%s*end%-method%s*$" ---@type string Palavra-chave "end-method"
Lib.regex.key.end_class = "^%s*end%-class%s*$" ---@type string Palavra-chave "end-class"
...
Lib.regex.key.if_cond = "^%s*if (%w+ [engl][qet] %w+) then%s*$" ---@type string Sequência "if-then" com seu teste
Lib.regex.key.else_cond = "^%s*else%s*$" ---@type string Palavra-chave "else"
Lib.regex.key.end_cond = "^%s*end-if%s*$" ---@type string Palavra-chave "end-if"
...
Lib.regex.class.def = "^%s*class (%w+)%s*$" ---@type string Captura o nome de uma declaração de classe
Lib.regex.class.method = "^%s*method (%w+)%(([%w%s,]*)%)%s*$" ---@type string Captura o nome e a string com os parâmetros de um método se existirem
...
Lib.regex.op.arith = "^%s*(%w+)%s*([%+-%*/])%s*(%w+)%s*$" ---@type string Captura as variáveis e operadores envolvidos em uma operação
Lib.regex.op.method = "^%s*(%w+)%.(%w+)%(([%w%s,]*)%)%s*$" ---@type string Captura o objeto, método e parâmetros numa chamada de método
